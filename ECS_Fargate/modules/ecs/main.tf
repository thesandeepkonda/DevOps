...
resource "aws_ecr_repository" "this" {
  for_each = {
    for k, v in var.services : k => v if v.create_repo
  }
...
data "aws_ecr_repository" "this" {
  for_each = {
    for k, v in var.services : k => v if v.create_repo == false
  }
  name = each.value.repo_name
}
...
locals {
  ecr_urls = merge(
    ...
    { for k, v in data.aws_ecr_repository.this : k => v.repository_url }
  )
}
...
resource "aws_ecs_task_definition" "this" {
  ...
  network_mode             = var.is_fargate ? "awsvpc" : var.task_network_mode
  ...
  container_definitions = jsonencode([{
    ...
    portMappings = [{
      ...
      hostPort      = var.is_fargate ? each.value.container_port : each.value.host_port
    }]
    secrets = [
...
resource "aws_ecs_service" "this" {
  ...
  launch_type     = var.is_fargate ? "FARGATE" : null
  ...
  dynamic "network_configuration" {
    for_each = (var.task_network_mode == "awsvpc" || var.is_fargate) ? [1] : []
    content {
